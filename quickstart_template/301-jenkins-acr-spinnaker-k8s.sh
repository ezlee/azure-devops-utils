...
function print_usage() {
  cat <<EOF
https://github.com/ezlee/azure-quickstart-templates/tree/master/301-jenkins-acr-spinnaker-k8s
Command
  $0
Arguments
...
function install_kubectl() {
  if !(command -v kubectl >/dev/null); then
    ...
    sudo curl -L -s -o $kubectl_file https://storage.googleapis.com/kubernetes-release/release/v1.7.7/linux/amd64/kubectl
...
function install_az() {
  if !(command -v az >/dev/null); then
    ...
    sudo apt-get -y update && sudo apt-get install -y apt-transport-https azure-cli
  fi
}
...
#Set defaults
pipeline_port="8000"
artifacts_location="https://raw.githubusercontent.com/ezlee/azure-devops-utils/master/"
